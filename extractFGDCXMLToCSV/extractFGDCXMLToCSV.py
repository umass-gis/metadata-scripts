# This script iterates through multiple XML files generated by the "Export Metadata Multiple" tool in ArcMap, retrieves each item's title and bounding coordinates, and generates a single CSV file with this information. The script is designed to extract relevant information about a georeferenced raster (aerial photo), and so it will need to be customized before the script can be used for other XML files.
# Script based on https://www.geeksforgeeks.org/convert-xml-to-csv-in-python/ with help from https://stackoverflow.com/questions/63814128/iterate-through-folders-and-find-a-file-to-put-into-a-dataframe
...
# Define the working directory - update this with the location of the XML files if they are in a different folder than where this python script is stored
directoryPath = '.'
...
cols = ["title", "identifier", "west", "east", "north", "south", "bbox"]
rows = []
        ...
        with open(file, 'r') as f:
            ...
            for bounding in root.findall("./idinfo/spdom/bounding"):
                ...
                this_df = pd.DataFrame([{"title": title,
                             ...
                             "north": northbc,
                             "south": southbc,
                             "bbox": ("ENVELOPE(" + westbc + ", " + eastbc + ", " + northbc + ", " + southbc + ")")}], columns=cols)
